...
2. go through entire file line by line
	...
	c. -Processing-
        ...
        Find the tonic key
        Count cadences (see "Steps to find cadence" below)

Steps to find cadence:
...
4. Figure out intervals based on semitones
	a. e.g. 0 is I, 2 is II, 4 is III, 5 is IV, 7 is V, 9 is VI, 10 is VII
	b. what to do with 1, 3, 5, 8, 11?
5. Count up each cadence pattern I-IV or V-I or I-vi, etc.
...
with open('chords_uku_english_only_songmarkers_empty_lines_removed.txt', 'r') as f:
    curLine = f.readline()
    while (curLine!=""):
        #--STAGE 0: RESET--
        origChords = []
        ...
        if(len(origChords)>0):
            ...
            for i in range(0, len(numChords) - 1):  # if we're doing every pair, we want to skip last element for loop
                cadenceFound = False;
                curCadence = numChords[i] + "-" + numChords[i + 1]
                #loop thru output table (a list of lists) and add 1 if it's there, add it to table if not there
                for j in range(0, len(cadences)):
                    if(cadences[j][0]==curCadence):
                        cadences[j][1]+=1;
                        cadenceFound = True;
                        break;
                if(not cadenceFound):
                    cadences.append([curCadence, 1]) #add cadence and set to 1
        curLine = f.readline();

#we're done.  Now print
cadences_sorted = sorted(cadences, key=lambda innerList: innerList[1]) #sort by count
for cadence in cadences_sorted:
    print("Cadence: "+cadence[0]+"\t\tCount: "+str(cadence[1]));

with open("cadences_counted.txt","wb") as myFile:
    for cadence in cadences_sorted:
        myFile.write("Cadence: "+cadence[0]+"\t\tCount: "+str(cadence[1]));